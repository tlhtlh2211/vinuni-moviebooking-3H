...
export async function GET(request: NextRequest) {
  try {
    ...
    if (response.status === 200) {
      ...
      return NextResponse.json({ error: "Failed to fetch revenue summary" }, { status: response.status })
    }
  } catch (error: any) {
    console.error("Error fetching revenue summary:", error)
    if (error.response) {
      return NextResponse.json(
        { error: error.response.data.message || "Failed to fetch revenue summary" },
