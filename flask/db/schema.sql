...
BEGIN
    ...
    SELECT s.seat_id
     ...
     WHERE s.screen_id = v_screen_id
       AND s.seat_id IN (SELECT seat_id FROM JSON_TABLE(p_seat_ids, '$[*]' COLUMNS (seat_id INT PATH '$')) AS jt)
    ...
    SELECT COUNT(*) INTO v_cnt
      FROM seats s
      JOIN JSON_TABLE(p_seat_ids, '$[*]' COLUMNS (seat_id INT PATH '$')) j5
        ON j5.seat_id = s.seat_id
     WHERE s.screen_id <> v_screen_id;
    IF v_cnt > 0 THEN
        SIGNAL SQLSTATE '45000'
    ...
    SELECT COUNT(*) INTO v_cnt
      ...
      JOIN JSON_TABLE(p_seat_ids, '$[*]' COLUMNS (seat_id INT PATH '$')) j6
        ON j6.seat_id = t.seat_id
    ...
    SELECT COUNT(*) INTO v_cnt
      FROM JSON_TABLE(p_seat_ids, '$[*]' COLUMNS (seat_id INT PATH '$')) j2
      LEFT JOIN seat_locks l
        ON l.seat_id = j2.seat_id
    ...
    DELETE FROM seat_locks
     WHERE user_id = p_user_id
       ...
       AND seat_id IN (SELECT seat_id
                         FROM JSON_TABLE(p_seat_ids,'$[*]' COLUMNS (seat_id INT PATH '$')) AS j4);
...
-- VIEW: Admin revenue summary for dashboard
-- Provides aggregated revenue metrics across different time periods
-- Single-row result with comprehensive revenue analysis
...
-- View: v_top_performing_movies
-- Purpose: Leaderboard of top performing movies with detailed metrics
-- Used by: Admin analytics dashboard for movie performance insights
